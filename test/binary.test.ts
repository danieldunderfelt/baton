...
describe.skipIf(!BUILT)("compiled binary", () => {
  ...
  test("ratings export writes an explicit snapshot into the scope", async () => {
    const dir = scope();
    const proc = Bun.spawn([BINARY, "ratings", "export"], {
      cwd: dir,
      env: { ...process.env, BATON_CONFIG_DIR: dir },
      stdout: "pipe",
      stderr: "pipe",
    ...
    expect(yaml).toContain("Exported by Baton");
  ...
  test("adapters list and duel list run in a fresh scope", async () => {
    ...
    expect(adapters.stdout).toContain("built-in");
