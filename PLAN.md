...
### Route blocks (user-owned deny list)

Identity stays delegated to the environment: Baton runs what it inherits and never asks whose account is behind a route. The cost of that stance is that a reachable route can be one the user must not spend — a client's enterprise seat logged into a multi-provider host, say. So the user gets a standing veto, written only through the trusted CLI, exactly like the authority ceiling.
...
3. **Approval precedes execution.** `baton adapters review <name>` in the trusted CLI shows the exact executable, argv, and env names; only after the user approves does Baton run its canary ("reply with exactly TOKEN") to verify extraction, then activate. Re-canary after a version bump re-runs the already-approved spec; a changed spec goes back through review. The whole lifecycle is keyed on a **spec digest** (sha256 of the canonical spec), not on the app name: `approve` must quote the digest the review printed, and the canary activates only the spec it launched for, so a resubmission mid-flight can never inherit someone else's approval. Approval also refuses to run when stdin is not a tty, with no override flag — and the review says plainly how far that reaches: it stops a stray tool call or a pasted command, not a local agent that already holds full shell access and could fake a terminal. That is the environment-transparency stance, not a gap: a user who delegated full shell access already extended that trust, and what the digest still buys them is that nobody can be *shown* one spec and made to approve another.
...
- 2026-08-25 (route blocks) — user decision: keep "Baton does not verify identity" exactly as it is, and add a config hatch beside it. The rule was never that every reachable route is fair game; it was that Baton does not *infer* whose account it is holding, and a deny list is the user stating what inference could not have told it. Decisions taken while landing it: patterns are globs over `<app>:<instance>/<slug>` — the fingerprint's own prefix, so what is blocked reads the same as what is selected and no second addressing vocabulary is invented; the instance segment defaults to `*`, since a route worth blocking on one account is rarely fine on another; a block beats *every* relaxation (last-resort selection, session affinity on resume, the canary), because a hatch with an exception is not a hatch; `baton block add` prints the routes it currently matches, since a deny list that silently matches nothing is the failure mode this feature can least afford; and there is deliberately no MCP tool that writes one — the delegating agent is exactly who the list is aimed at.
- 2026-08-25 (phase-3 build) — decisions taken while landing duels/BT, discovery, resume, cursor-agent, HTTP and the conformance suite:
  ...
  - **Blindness is a property of the unjudged duel, not a stored flag.** `winner IS NULL` is the whole test, so judging is what un-blinds, and every surface that renders a run owes the check: `get_run` returns status/output/timings only (model, app, slug, instance, attempt targets, sessionRef and the error text withheld — "codex exited 1" names the app as surely as the model field), `resume_run` and `report_result` refuse, and `baton runs` masks the model cell, the route line and the attempt targets. The prompt, cwd and category are *not* withheld: both sides share them, so they identify nobody.
  - **A tie is one directionless comparison** (`wins_a: 0, wins_b: 0, ties: 1`), not half a win each way; the `TIE_WEIGHT` split lives only in the fitter, so every judged duel is exactly one unit of comparison mass. Consequently the half-life guard counts `bt_edges` alongside the accumulator and reliability — a duel-only scope was silently reinterpreting decayed edge sums under a new curve — and `--reset-evidence` clears all three in one `BEGIN IMMEDIATE`.
  - **BT priors resolve per (model, category)** with a fallback to the uncategorised prior, and carry an *effective* pseudo-edge weight (shrinkage × capped stored weight × decay from `as_of` × profile weight), so a thin, stale or muted prior degrades smoothly toward "no opinion" while the fit stays identified. `nEff` is Kish `(Σw)²/Σw²` (migration v7 adds `bt_edges.mass2`, decayed by the square of the factor), and `se` comes from the regularized information matrix pushed through the same centering transform the thetas take — the previous numbers counted events, not evidence.
  - **A resume cannot raise the authority its session was created under.** The executed authority is the min of what the caller asked for, what the origin run resolved, and the scope's current ceiling — a ceiling widened after the fact does not retroactively promote a live session. Two neighbours: one live turn per session (a second concurrent resume is refused, naming the run that holds it), and a resume through an adapter whose version or digest moved is refused rather than re-entering a different program with the old session handle.
  - **No `tasks` capability is advertised.** SDK v2 exposes the task wire *types* but no server-side runtime — `tasks/*` is `Exclude`d from both `RequestMethod` and `RequestTypeMap`, so it cannot be registered as a handler at all. Advertising it would be a promise the server cannot serve; `wait: false` + `get_run` stays the only polling path, which is what the open question above anticipated.
  - Exit criterion rehearsed as an ordinary test (`test/onboarding.e2e.test.ts`): an unknown app is discovered, registered, quarantined, reviewed and approved through the real MCP and CLI surfaces, canaried into `active`, delegated to, and seeded — no config file edited, no live CLI, no quota. It drives the **source entry point**, never `dist/baton`: approval can only run in-process (it refuses without a tty), and mixing a compiled subprocess with in-process source in one flow made a stale build report the old CLI's behaviour as green. The compiled artifact is covered by `test/binary.test.ts`, which is where a build that never happened should fail.
...
- 2026-08-25 (phase-3 code review — both reviewers via Baton; kimi in a pinned worktree) — gpt-5.6-sol (full surface) and kimi-k3 (targeted: math, templates, http, export), both: fix first. Sol, reproduced live: TOCTOU race activating an unreviewed spec mid-canary (fixed: digest-bound CAS through the whole lifecycle); terminal-escape spoofing of the approval view (fixed: control/bidi chars rejected in every field, argv rendered as JSON, resume shown, digest displayed); duel blindness leaking through `get_run`/CLI (fixed: blind runs redacted until judged, incl. error text); agent-callable approval (fixed: TTY + digest confirmation, limits documented honestly — a full-permission local agent already has the shell). Sol also: resume could raise authority/timeout (now clamps to the original resolution), concurrent-resume lease, adapter-revision binding on resume, canary exact-match, discovered apps excluded from `max_autonomy`, stale version cache. Convergent sol+kimi with exact numeric repros: tie double-mass (one tie = one comparison now), `bt_edges` missing from the half-life guard, BT priors flattened (now per-category, weight- and as_of-decayed, profile-scaled), dishonest nEff/se (per-edge Σw² via migration v7; full-information covariance). Kimi additionally: post-fit recentering dragged prior-only models off their priors when others duelled — recentering removed, thetas reported anchored (0 = the neutral grade-3 opponent), `converged` flag added; template gaps (hop cap now in the shared core, kimi fallback wording); HTTP shutdown drains in-flight handlers and documents the standing-capability tradeoff; profile export re-validates canonical ids and flags free-text categories.
