...
- **MCP server** (`baton mcp`): stdio per host by default. Built on the published TypeScript SDK (1.30.0, protocol `2025-11-25` — the 2026-07-28 stateless core has no published SDK yet; build-time finding). Baton keeps the stateless discipline at its own layer regardless: cross-call state travels as server-minted handles (`run_id`) in tool arguments; long-running work via handle polling, upgrading to the Tasks extension when host+SDK support lands. `baton serve --http` (phase 3) runs the same server as stateless Streamable HTTP — one daemon per environment scope, since a daemon inherits one environment.
...
Setup: `baton instance add claude-code personal-2 --env CLAUDE_CONFIG_DIR=~/.claude-personal2`; login stays interactive (Baton prints the command, the user completes OAuth once), followed by an optional canary run as a sanity check. Instance definitions live in the active `BATON_CONFIG_DIR`, so each scope only knows the instances its config defines. **Cursor Agent is excluded from instances and pools**: its only identity mechanism is an API key; no env-overlay mechanism, no support, revisit if that changes.
