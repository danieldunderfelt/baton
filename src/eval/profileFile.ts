...
/**
 * The shareable profile file (PLAN.md §Layering and sharing). A profile is
 * **priors only, canonical models only**: no execution targets, no instances,
 * no machine details, no prompts. That is the portability guarantee, so the
 * parser rejects any key outside the format rather than ignoring it — a file
 * carrying local fingerprints is a bug in whatever wrote it, not something to
 * silently drop.
 *
...
export function importProfileFile(
  ...
  const doc = parseProfileDocument(read(filePath), filePath);
  const profile = opts.name ?? doc.name;
  // Provenance is the document's own name: renaming locally must not hide where
  // the numbers came from.
  const diff = importPriors(db, profile, priorEntriesOf(doc), doc.name, at);
...
export function diffProfileDocument(
  ...
  return diffPriors(db, profile, priorEntriesOf(doc), { source: `imported:${doc.name}`, at });
}
...
export function parseProfileDocument(text: string, source = "<input>"): ProfileDocument {
...
function validate(raw: Record<string, unknown>, source: string): ProfileDocument {
  rejectUnknown(raw, DOC_KEYS, source, "profile");
  const name = raw.name;
  if (typeof name !== "string" || !name.trim()) {
    throw new Error(`${source}: profile needs a non-empty "name".`);
  }
  const exportedAt = raw.exported_at ?? nowIso();
